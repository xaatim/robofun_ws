...
class MotorEncoder:
    ...
    def set_angular_velocity(self, angular_velocity):
        ...
        self.duty = float(np.clip(0.1 * self.target_angular_velocity, -1.0, 1.0))
...
class RobotDriver:
    def __init__(self, motor_addrs=[0x43, [0, 1], [2, 3]], encoder_addrs=[0x40, 0x41], hz=10, motor_type="hw231"):
        ...
        self.left_motor = MotorEncoder(
            self.i2c_bus_id, motor_addr=motor_addrs[0], motor_pins=motor_addrs[1], encoder_addr=encoder_addrs[0], encoder_invert=True, motor_type=motor_type)
        self.right_motor = MotorEncoder(
            self.i2c_bus_id, motor_addr=motor_addrs[0], motor_pins=motor_addrs[2], encoder_addr=encoder_addrs[1], encoder_invert=False, motor_type=motor_type)
    ...
    def _move_thread(self):
        while not self.stop_move:
            ...
            time.sleep(
                sorted([self.wait_time-((time.monotonic_ns()-start_time)/1e9), 0])[1])
    ...
    def _get_kinematic_move_calculation(self):
        ...
        mC = np.array([self.left_motor.get_angular_velocity(),
                      self.right_motor.get_angular_velocity()])
    ...
    def _calculate_robot_position(self):
        ...
        dtheta = (right_wheel_distance - left_wheel_distance) / self.robot_base
        self.global_position[0] = self.global_position[0] + \
            (base_travel_distance * np.cos(self.heading))  # X
        self.global_position[1] = self.global_position[1] + \
            (base_travel_distance * np.sin(self.heading))  # Y
        self.heading = (self.heading + dtheta + np.pi) % (2*np.pi) - np.pi
    ...
    def move(self, movement):
        ...
        self.left_motor.set_angular_velocity(target_speed[0])
        self.right_motor.set_angular_velocity(target_speed[1])
    ...
    def retrieve_i2c_id(self):
        detect_result = subprocess.run(
            ['i2cdetect', '-l'], stdout=subprocess.PIPE)
        if 'i2c-tiny-usb' in detect_result.stdout.decode():
            i2c_device = subprocess.run(
                ['grep', 'i2c-tiny-usb'], input=detect_result.stdout, stdout=subprocess.PIPE)
            i2c_bus = subprocess.run(
                ['awk', '{print $1}'], input=i2c_device.stdout, stdout=subprocess.PIPE).stdout.decode()
